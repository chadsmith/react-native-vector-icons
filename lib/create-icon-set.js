...
function createIconSet(glyphMap : Object, fontFamily : string, fontFile : string) : Function {
  ...
  var TabBarItem = React.createClass({
    ...
    updateIconSources: function() {
      let { iconSize, iconName, color, selectedIconName, selectedColor } = this.props;
      var size = iconSize || TAB_BAR_ICON_SIZE;
      if(iconName) {
        getImageSource(iconName, size, color).then(icon => this.setState({ icon }));
      }
      if(selectedIconName || selectedColor) {
        getImageSource(selectedIconName || iconName, size, selectedColor).then(selectedIcon => this.setState({ selectedIcon }));
      }
    },
